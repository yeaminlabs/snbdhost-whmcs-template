            ...
            <table class="table table-hover align-middle mb-0" style="border-collapse: collapse; border: none;">
                ...
                <tbody>
                    ...
                    <tr onclick="window.location='viewinvoice.php?id={$invoice.id}'" style="cursor: pointer; transition: all 0.15s ease;">
                        <td class="py-3 px-4">
                            <span class="fw-bold text-danger">{if $invoice.invoicenum|strpos:'#' === 0}{$invoice.invoicenum}{else}#{$invoice.invoicenum}{/if}</span>
                        </td>
                        ...
                        <td class="py-3 px-4 text-end" onclick="event.stopPropagation();">
                            <a href="viewinvoice.php?id={$invoice.id}" class="topbar-icon-btn me-1" style="width: 44px; height: 44px; display: inline-flex; align-items: center; justify-content: center;" title="View Invoice" aria-label="View Invoice {if $invoice.invoicenum|strpos:'#' === 0}{$invoice.invoicenum}{else}#{$invoice.invoicenum}{/if}">
                                <i class="ti ti-eye" style="font-size: 1.05rem;"></i>
                            </a>
                            {if $invoice.status eq "Unpaid"}
                            <a href="viewinvoice.php?id={$invoice.id}" class="btn btn-brand-clean py-2 px-3" style="font-size: 0.75rem !important; border-radius: 8px !important; box-shadow: none !important; display: inline-flex; align-items: center; min-height: 44px;" title="Pay Now" aria-label="Pay Invoice {if $invoice.invoicenum|strpos:'#' === 0}{$invoice.invoicenum}{else}#{$invoice.invoicenum}{/if}">
